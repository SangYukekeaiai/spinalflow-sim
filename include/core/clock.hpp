...
#include "arch/output_queue.hpp"            // S0 (receive + line assembly)
...
#include "arch/driver/weight_lut.hpp"       // S3
...
public:
  ...
  // Declare which output spine is active for the current position.
  // All entries staged in this cycle are assembled under this spine.
  void SetActiveOutputSpine(std::uint16_t spine_id) { out_q_.SetActiveSpine(spine_id); }

  // S0->S1
  void SetSt0St1Valid(bool v) override { st0_st1_valid_ = v; }
  bool st0_st1_valid() const override  { return st0_st1_valid_; }

  // S1->S2
  void SetSt1St2Valid(bool v) override { st1_st2_valid_ = v; }
  bool st1_st2_valid() const override  { return st1_st2_valid_; }

  // S2->S3
  void SetSt2St3Valid(bool v) override { st2_st3_valid_ = v; }
  bool st2_st3_valid() const override  { return st2_st3_valid_; }

  // NEW: S3->S4
  void SetSt3St4Valid(bool v) override { st3_st4_valid_ = v; }
  bool st3_st4_valid() const override  { return st3_st4_valid_; }

  // NEW: S4->S5
  void SetSt4St5Valid(bool v) override { st4_st5_valid_ = v; }
  bool st4_st5_valid() const override  { return st4_st5_valid_; }

  // Config / params (unchanged)
  ...
  // Batch/FIFO context for S3/S4 (unchanged)
  ...
  // DRAM segment fetcher for S5 (unchanged)
...
private:
  ...
  // Pipeline handshakes
  bool st0_st1_valid_ = true;  // S0 publishes
  bool st1_st2_valid_ = true;  // S1 publishes
  bool st2_st3_valid_ = true;  // S2 publishes
  bool st3_st4_valid_ = true;  // NEW: S3 publishes
  bool st4_st5_valid_ = true;  // NEW: S4 publishes
  // S3/S4 shared state
