...
bool ClockCore::run() {
    const bool s0 = out_q_.run();               // Stage 0
...
ClockCore& ClockCore::SetOutputSink(std::function<bool(const Entry&)> sink) {
    out_sink_ = std::move(sink);
    return *this;
}

bool ClockCore::SendToOutputSink(const Entry& e) {
    return out_sink_ ? out_sink_(e) : false;
}
