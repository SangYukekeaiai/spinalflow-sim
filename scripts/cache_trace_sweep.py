#!/usr/bin/env python3
"""
Sweep a model directory for cache_traces CSVs and generate selected figures.

CLI
  1) --model-dir  Path to a model directory, e.g. stats/repo4/vgg16/
  2) --plots      Which figures to generate (comma-separated). Examples:
                  small_rates, overall_counts, perpos_counts, heatmaps, all
     Default:     small_counts,overall_counts,heatmaps
  3) Skips any figure that already exists.

Notes
  - Uses scripts/cache_trace_plots.py under the hood (imports its process()).
  - For per-position plots, checks every expected PDF (<hout>_<wout>_*.pdf).
  - For heatmaps, both hit and miss PDFs must exist to skip.
"""
...
# Ensure we can import scripts.cache_trace_plots regardless of CWD
sys.path.insert(0, _repo_root())
from scripts.cache_trace_plots import (
...
def _find_cache_trace_csvs(model_dir: str) -> List[str]:
    csvs: List[str] = []
...
def _expected_outputs(csv_path: str, plots: List[str]) -> Dict[str, List[str]]:
    """Return a mapping plot_key -> expected file list for that plot."""
    outdir = os.path.splitext(csv_path)[0]
    exp: Dict[str, List[str]] = {}
    # Simple ones
    if "small_counts" in plots:
        exp["small_counts"] = [os.path.join(outdir, "small_multiples_counts.pdf")]
    if "small_rates" in plots:
        exp["small_rates"] = [os.path.join(outdir, "small_multiples_rates.pdf")]
    if "overall_counts" in plots:
        exp["overall_counts"] = [os.path.join(outdir, "overall_per_timestep_counts.pdf")]
    if "overall_rates" in plots:
        exp["overall_rates"] = [os.path.join(outdir, "overall_per_timestep_rates.pdf")]
    if "heatmaps" in plots:
        exp["heatmaps"] = [
            os.path.join(outdir, "heatmaps_hit_rate.pdf"),
            os.path.join(outdir, "heatmaps_miss_rate.pdf"),
        ]
    # For per-position we need to read CSV to enumerate positions
    ...
    if need_perpos:
        ...
        if "perpos_counts" in plots:
            exp["perpos_counts"] = [os.path.join(outdir, f"{h}_{w}_counts.pdf") for (h, w) in pos_set]
        if "perpos_rates" in plots:
            exp["perpos_rates"] = [os.path.join(outdir, f"{h}_{w}_rates.pdf") for (h, w) in pos_set]
    return exp


def process_model_dir(model_dir: str, plots: List[str], per_page: int) -> None:
    expanded = _expand_plots_list(plots)
    csvs = _find_cache_trace_csvs(model_dir)
    if not csvs:
        print(f"[info] No cache_traces CSVs under: {model_dir}")
        return
...
def main(argv: List[str]) -> int:
    p = argparse.ArgumentParser(description="Sweep cache_traces CSVs and generate selected figures.")
    p.add_argument(
        "--model-dir",
    ...
    p.add_argument(
        ...
        help=(
            ...
            "all, perpos, perpos_counts, perpos_rates, small, small_counts, small_rates, heatmaps, overall, overall_counts, overall_rates"
        ),
    )
